function AllData = add_LFP_adc(AllData, ls, oe, analog_events, task_counter)
...
for i = 1:numel(AllData.trials)
    ...
    analog_time_event_in_trial            = get_event_in_boundary(trial_time_event_in_trial, analog_events(1).time_sample);
    ...
    if ~isempty(analog_time_event_in_trial)
        AllData.trials(i).analog_event    = double(analog_time_event_in_trial - trial_time_event_in_trial(1))/fs_raw;
    end
end
end
%%
