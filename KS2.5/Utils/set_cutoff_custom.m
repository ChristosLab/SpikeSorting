function rez = set_cutoff_custom(rez)
    ...
    while Th>=ops.Th_cutoff(2)
      % continually lower the threshold, while the estimated unit contamination is low
        ...
        if Q>fcontamination || R>.05 % if the unit is already contaminated, we break, and use the next higher threshold
            % Edit: For MUA clusters that fail to show good enough
            % refraction even with the most strict cutoff, allow more
            % spikes to go through instead, using a universal cutoff
            % threshold.
            if Th==ops.Th_cutoff(1)
                Th = ops.Th_cutoff(2) - .5; % Offset the threshold step recovery following the while loop
            end
            break;
        else
            if Th==ops.Th_cutoff(1) && Q<.05
