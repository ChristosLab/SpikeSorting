% REMOVE_KS25_DUPLICATE_SPIKES:
% Edited from REMOVE_KS2_DUPLICATE_SPIKES to:
% TODO:
% 1) Reconstitute spike waveform, depth and amplitude from whitened and
% shifted data.
% 2) Estimate spike depth from the templates' amplitude center of mass
% instead of maximum
% -ZW 3/28/2023
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
function rez = remove_ks25_duplicate_spikes(rez, varargin)
    ...
    %%%%%%%%%%%%%
    %   To be compliant with the changes made to rezToPhy.m in
    %   https://github.com/MouseLand/Kilosort/blob/ad462bd78010f1f872fffefa2cfe31e649361da2/utils/rezToPhy.m#L69-L71J
    %   The template (average waveform) is now computed on the preprocessed
    %   traces (highpass filtering, whitening and shifting), the same as
    %   observed using the rezToPhy outputs (in Phy, for instance).
%     whiteningMatrix = rez.Wrot/rez.ops.scaleproc;
%     whiteningMatrixInv = whiteningMatrix^-1;
    whiteningMatrixInv = eye(size(rez.Wrot));
    %%%%%%%%%%%%%
    ...
    %   spike_primary is the channel for depth
