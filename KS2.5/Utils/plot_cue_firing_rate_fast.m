function plot_cue_firing_rate_fast(filename)
...
fix_spike_count = zeros(numel(target_cells), numel(target_trials));
for i_target_cell = 1:numel(target_cells)
    i = target_cells(i_target_cell);
    fix_spike_count(i, :) = cell2mat(arrayfun(@(x) sum(x.ss(target_samples, i)), MatData.trials(target_trials), 'UniformOutput', false));
end
fix_fr = fix_spike_count/numel(target_samples) * MatData.sample_rate;
